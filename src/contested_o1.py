...
def o1_parallel(messages):
    try:
        ...
        try:
            ...
            logger.info(f'o1 second openai api error: task_id:{task_id}, test_id:{test_id} exception:{traceback.format_exc()}')
            gpt_output = 'error'
    return gpt_output
def o1_fix_tests(prompt, input, task_id, test_id):
    global logger
    global canonical_code_test_output
    instruction = "Given a docstring of a Python function and the input of one test case, you need to generate the output of the test case based on the description of the function. \n The function is defined as follows:\n" + prompt + "\n\nThe input of the test case is:\n" + str(input) + "\n\nPlease generate the output of the test case. Do not provide any explanations. Use  ```python``` to surround the output of test. Don't use ```python``` to surround other things except the output of test. If the output is string type, you should use proper quotes to surround the string. If the test input is invalid or you meet any error, please return 'error' within ```python```."
    messages = [
    ...
    gpt_output = o1_parallel(messages)
    ...
    for gpt_output in gpt_outputs:
        results = re.findall(pattern, gpt_output, flags=re.DOTALL)
        if len(results) < 1:
            cur_res = 'other_error'
        elif results[0].strip() in ['error', '"error"', "'error'"]:
            cur_res = 'o1_error'
        else:
            try:
                fix_output = eval(results[0].strip())
                cur_res = results[0].strip()
            except:
                cur_res = 'eval_error'
        if cur_res not in total_results:
            total_results[cur_res] = 1
        else:
            total_results[cur_res] += 1
    total_results = sorted(total_results.items(), key=lambda x: x[1], reverse=True)
    rank_1_res = total_results[0][0]
    # print(rank_1_res)
    if rank_1_res == 'other_error':
        return 'other_error', 'other_error'
    if rank_1_res == 'o1_error':
        logger.info(f'o1 fix o1_error: task_id:{task_id}, test_id:{test_id}')
        return 'o1_error', 'o1_error'
    if rank_1_res == 'eval_error':
        logger.info(f'o1 fix eval_error: task_id:{task_id}, test_id:{test_id}')
        return 'eval_error', 'eval_error'
    try:
        fix_output = eval(rank_1_res)
    except:
        return 'eval_error', 'eval_error'
    fix_flag = fix_output == canonical_code_test_output[task_id][test_id]
    logger.info(f'o1 fix success: task_id:{task_id}, test_id:{test_id} fix_output:{fix_output}, ref_output:{canonical_code_test_output[task_id][test_id]}, o1_fix_flag:{fix_flag}, total_results:{total_results}, rank1_res:{rank_1_res}')
    return 'true', fix_output
...
if __name__ == '__main__':
    ...
    if not os.path.exists(f"../{runtime_dir}/{data_type}/code_test_res.pkl"):
        ...
        total_code_test_res_raw = evaluate_with_test_cases(solutions, total_test_dict, 1, assert_format=False, logger=logger1, have_entry=True)
    ...
    for task_id in total_code_test_res_all:
        ...
        # passed_tests_num = dict(sorted(passed_tests_num.items(), key=lambda x:x[1], reverse=True))
    ...
    for task_id in tqdm(total_solution_dict):
        ...
        try:
            ...
            human_test_oracle_gt_dict = {}
            # {test_id: oracle}}
            if not os.path.exists(f"../runtime/{data_type}/total_human_test_oracle_gt_dict_o1/"):
                os.makedirs(f"../runtime/{data_type}/total_human_test_oracle_gt_dict_o1/")
            _task_id = task_id.replace('/', '_')

            if os.path.exists(f"../runtime/{data_type}/total_human_test_oracle_gt_dict_o1/{_task_id}.pkl"):
                print('load from o1')
                human_test_oracle_gt_dict = load_pickle(f"../runtime/{data_type}/total_human_test_oracle_gt_dict_o1/{_task_id}.pkl")
            ...
            while len(unknown_tests) > 0:
                ...
                for cur_code_id in code_test_output:
                    ...
                    if cur_code_test_output != canonical_code_test_output[task_id][consider_test_id]:
                        any_need_fix = True
                ...
                if len(outputs_on_test) == 1 and all([x == None for x in fail_reasons]):
                    ...
                    logger.info(f'---skip iteration, any_need_fix:{any_need_fix} after check, need fix 0 codes, find_enough_codes: {find_enough_codes}---task_id:{task_id}, epoch: {iter_num}, pass_num_dict:{sorted_pass_num_dict}, total_test_num:{total_test_num}')
                ...
                if len([each for each in fail_reasons if each != None]) > len(fail_reasons) * 0.9:
                    o1_fix_flag, o1_fix_output = 'judge_error', 'judge_error'
                    cur_oracle_gt = o1_fix_output
                    if isinstance(canonical_fail_reason[task_id], str):
                        cur_ref_fail_reason = canonical_fail_reason[task_id]
                    else:
                        cur_ref_fail_reason = canonical_fail_reason[task_id][consider_test_id]
                    whehter_correct = cur_ref_fail_reason != None
                    logger.info(f'---judge error --test_id:{consider_test_id} ---task_id:{task_id}, epoch: {iter_num}, whehter fail correct:{whehter_correct}')

                else:
                    if consider_test_id in human_test_oracle_gt_dict and not (human_test_oracle_gt_dict[consider_test_id][0] == 'true' and human_test_oracle_gt_dict[consider_test_id][1] == 'error'):
                    # if False:
                        print('load from saved')
                        o1_fix_flag, o1_fix_output = human_test_oracle_gt_dict[consider_test_id]
                        cur_oracle_gt = o1_fix_output
                        logger.info(f'o1 load: task_id:{task_id}, test_id:{consider_test_id} fix_output:{cur_oracle_gt}, o1_fix_flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}')
                    else:
                        o1_fix_flag, o1_fix_output = o1_fix_tests(dataset_dict[task_id]["prompt"], cur_input, task_id, consider_test_id)
                        if o1_fix_flag != 'other_error' and o1_fix_flag != 'eval_error':
                            human_test_oracle_gt_dict[consider_test_id] = (o1_fix_flag, o1_fix_output)
                        cur_oracle_gt = o1_fix_output
                
                if o1_fix_flag == 'judge_error':
                    iter_num -= 1
                if 'error' in o1_fix_flag:
                    logger.info(f'---ref_code_fail {cur_oracle_gt} --test_id:{consider_test_id} ---task_id:{task_id}, epoch: {iter_num}')
                ...
                with ProcessPoolExecutor(concurrent_num) as executor:
                    futures = []
                    ...
                    if len(new_filter_code_ids) == len(cur_fix_ids) == not_filtered_num: # or len(cur_fix_ids)
                        logger.info(f'---no fix correct, discard test --test_id:{consider_test_id} ---task_id:{task_id}, epoch: {iter_num}, o1_fix_flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}')
                        delete_tests.append(consider_test_id)
                        test_dict.pop(consider_test_id)
                        ...
                        for cur_code_id in code_test_output:
                            ...
                            if all(code_test_res[cur_code_id].values()):
                                already_satisfied_ids.append(cur_code_id)
                        logger.info(f'---o1 fix flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}, --test_id:{consider_test_id} ---task_id:{task_id}, epoch: {iter_num}, o1_fix_flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}, num_fix_ids:{len(cur_fix_ids)}, num_fix_fail_ids:{len(new_filter_code_ids)}, num_remain_code:{len([x for x in list(code_test_output.keys()) if x not in filter_code_ids])}')
                    else:
                        logger.info(f'---o1 fix flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}, --test_id:{consider_test_id} ---task_id:{task_id}, epoch: {iter_num}, o1_fix_flag:{o1_fix_output == canonical_code_test_output[task_id][consider_test_id]}, num_fix_ids:{len(cur_fix_ids)}, num_fix_fail_ids:{len(new_filter_code_ids)}, num_remain_code:{len([x for x in list(code_test_output.keys()) if x not in filter_code_ids])}')
            ...
            _task_id = task_id.replace('/', '_')
            pickle.dump(human_test_oracle_gt_dict, open(f"../runtime/{data_type}/total_human_test_oracle_gt_dict_o1/{_task_id}.pkl", 'wb'))
