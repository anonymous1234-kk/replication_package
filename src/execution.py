...
def evaluate_with_test_code( 
    ...
    with ProcessPoolExecutor(40) as executor:
        ...
        for sample in samples:
            ...
            have_entry = sample['have_entry']
            if solution_id in existed_completion[task_id]:
                continue
            existed_completion[task_id].add(solution_id)
            args = (task_id, solution_id, prompt, completion, test, entry_point, timeout, have_entry)
        ...
        for idx, future in enumerate(as_completed(futures)):
            ...
            results[result["task_id"]][result["solution_id"]] = result
    ...
    for sample in samples:
        ...
        solution_id = sample["solution_id"]
        completion = sample["completion"]
        result = results[task_id][solution_id]
...
def evaluate_with_test_cases(
    ...
    with ProcessPoolExecutor(30) as executor:
        ...
        for solution in solutions:
            ...
            if (task_id, solution_id) in existed_completion[task_id]:
                continue
            existed_completion[task_id].add((task_id, solution_id))
            task_test_cases = test_cases_dict[task_id]
            args = (task_id, solution_id, prompt, completion, task_test_cases, entry_point, timeout, multi_args, assert_format, logger, have_entry)
...
def evaluate_with_test_cases_serial(
    solutions,
    test_cases_dict,
    timeout,
    assert_format=False,
